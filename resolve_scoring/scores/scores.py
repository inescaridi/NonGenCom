...
def get_scored_dataframe(
    ...
    biosex_context: str,
    biosex_scenario: str,
    biosex_req_field: str,
    biosex_src_field: str,
    ...
    sum_prep = Summary(
        "prep_processing_seconds",
        "Time spent preparing the data",
        registry=prometheus_registry,
    )
    sum_merge = Summary(
        "merge_processing_seconds",
        "Time spent processing merge",
        registry=prometheus_registry,
    )
    sum_score = Summary(
        "score_processing_seconds",
        "Time spent processing score",
        registry=prometheus_registry,
    )
    sum_sort = Summary(
        "sort_processing_seconds",
        "Time spent processing sort",
        registry=prometheus_registry,
    )
    ...
    with sum_prep.time():
        ...
        print_series(
            f"Biological sex prior ({biosex_score.context})", biosex_score.prior
        )
        print_series(
            f"Biological sex likelihood ({biosex_score.scenario})",
            biosex_score.likelihood,
        )
        print_series(f"Biological sex posterior", biosex_score.posterior)

        req_df["req_biosex"] = req_df[biosex_req_field].map(
            biosex_settings.mapping
        )

        src_df["src_biosex"] = src_df[biosex_src_field].map(biosex_settings.mapping)
    ...
    with sum_sort.time():
        ...
        scored_df.sort_values(
            biosex_settings.score_colname, ascending=False, inplace=True
        )
