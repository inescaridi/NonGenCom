...
if __name__ == '__main__':
    ...
    # for each variable we set up an instance, and apply the add_mp_score method
    ...
    result = (merged_dbs
              .pipe(BiologicalSex("Uniform", "High", "Perfect representation").add_mp_score, "FC estimate Biological Sex", "MP estimate Biological Sex")
              .pipe(Age().add_mp_score, "FC estimate Age Minimum (years)", "FC estimate Age Maximum (years)", "MP estimate Age Minimum (years)", "MP estimate Age Maximum (years)")
              .pipe(Date("2020-02-01", "2022-12-01", 120).add_mp_score, "FC Estimated Date of Death", "FC Estimated Date of Burial", "MP Estimated Date of Death", "MP Estimated Date of Burial")
              .pipe(Body("Uniform", "Standard", "Standard", "Torso/Disease").add_mp_score, "Torso/Disease", "MP Torso/Disease")
              .pipe(Height().add_mp_score, "FC height min estimated", "FC height max estimated", "MP height min estimated", "MP height max estimated")
              )
    ...
    result.to_csv("examples/results/mp_scoreCalculator_example.csv", index=False)
